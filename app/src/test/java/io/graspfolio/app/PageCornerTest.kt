...
class PageCornerTest {
    @Test fun menuEnlargesBothDimensionsWithoutActivatingSpine() {
        ...
        for ((point, direction) in listOf(Offset(120f, 120f) to -1, Offset(880f, 120f) to 1)) {
            assertEquals(0, cornerDirection(point, bounds, 92f))
            assertEquals(direction, cornerDirection(point, bounds, 92f * 1.5f))
        }
    ...
    @Test fun spreadHasOnlyOuterCornersIncludingBlankCoverAndEndSlots() {
        ...
        assertEquals(-1, cornerDirection(Offset(110f, 60f), bounds, 92f))
        assertEquals(1, cornerDirection(Offset(1090f, 60f), bounds, 92f))
        for (x in listOf(510f, 590f, 600f, 610f, 690f))
            assertEquals(0, cornerDirection(Offset(x, 60f), bounds, 92f))
    }
    ...
    @Test fun narrowSpreadStillKeepsSpineInactive() {
        ...
        assertEquals(-1, cornerDirection(Offset(1f, 1f), bounds, 92f))
        assertEquals(1, cornerDirection(Offset(99f, 1f), bounds, 92f))
        assertEquals(0, cornerDirection(Offset(49f, 1f), bounds, 92f))
        assertEquals(0, cornerDirection(Offset(51f, 1f), bounds, 92f))
    }
}
