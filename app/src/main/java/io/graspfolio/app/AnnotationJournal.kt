...
internal data class DurableInk(
    ...
    val revision: Long = 0, val syncedRevision: Long = 0
) {
...
/** Local-writer-thread only. Atomic per-edit records; checkpoints retain the old recovery format. */
internal class AnnotationJournal(private val file: File, private val identity: String) {
    ...
    init {
        if (file.exists() || File(file.path + ".bak").exists()) {
            ...
            require(root.optInt("journalVersion", 1) == 1) { "恢复日志版本不支持" }
            ...
            state = DurableInk(InkCodec.decode(root.getString("data"), identity),
        ...
        entries.filter { it.first > sequence }.forEach { (number, entry) ->
            ...
            require(root.getInt("version") == 1 && root.getString("document") == identity && root.getLong("sequence") == number)
    ...
    fun replace(strokes: List<InkStroke>): Int {
        ...
        return append(state.copy(strokes = strokes, revision = state.revision + 1), added, removed)
    }
    ...
    private fun append(next: DurableInk, added: List<InkStroke>, removed: List<String>): Int {
        ...
        val record = JSONObject().put("version", 1).put("document", identity).put("sequence", number)
    ...
    fun importRemote(strokes: List<InkStroke>, hash: String) {
        val next = DurableInk(strokes, hash, state.revision + 1, state.revision + 1)
    ...
    private fun writeCheckpoint(value: DurableInk) {
        val root = JSONObject().put("journalVersion", 1).put("sequence", sequence)
            .put("revision", value.revision).put("syncedRevision", value.syncedRevision)
            .put("data", InkCodec.encode(identity, value.strokes)).put("base", value.base ?: "").put("dirty", value.dirty)
